...
class DynamicalComponentsAnalysis(object):
    ...
    def fit_projection(self, X_train, X_val, Y_val_gt, writer, d=None, n_init=None):
        ...
        for ii in range(n_init):
            ...
            coef, pi = self._fit_projection(X_train, X_val, Y_val_gt, writer, d=d)
            #print("pi:", pi)
    ...
    def _fit_projection(self, X_train, X_val, Y_val_gt, writer, d=None, record_V=False):
        ...
        if self.use_scipy:
            ...
            for epoch in range(1500):
                ...
                reg_val = ortho_reg_fn(v, self.ortho_lambda)
                loss_no_reg = loss - reg_val
                writer.add_scalar('train/train DCA PI', -loss_no_reg, epoch)
                if epoch % 200 == 0:
                    print(epoch, ":", -loss_no_reg.item())
                loss.backward()
                optimizer.step()

                c = self.estimate_cross_covariance(X_val)
                loss = build_loss(c, d, self.ortho_lambda, self.block_toeplitz)(v)
                reg_val = ortho_reg_fn(v, self.ortho_lambda)
                loss_no_reg = loss - reg_val
                ...
                loss_no_reg = loss - reg_val
                writer.add_scalar('gt/gt DCA PI', -loss_no_reg, epoch)
