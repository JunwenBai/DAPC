...
class StochasticDecoderLayer(nn.Module):
    ...
    def __init__(self, size, self_attn, src_attn, feed_forward, dropout_rate, death_rate=0.0,
                 normalize_before=True, concat_after=False):
        """Construct an DecoderLayer object."""
    ...
    def forward(self, tgt, tgt_mask, memory, memory_mask, cache=None):
        ...
        if self.concat_after:
            tgt_concat = torch.cat((tgt_q, self.self_attn(tgt_q, tgt, tgt, tgt_q_mask)), dim=-1)
            if not coin:
                x = residual + self.concat_linear1(tgt_concat) * 0.0
            else:
                if self.training:
                    x = residual + self.concat_linear1(tgt_concat) / (1 - self.death_rate)
                else:
                    x = residual + self.concat_linear1(tgt_concat)
        else:
            if not coin:
                x = residual + self.dropout(self.self_attn(tgt_q, tgt, tgt, tgt_q_mask)) * 0.0
            else:
                if self.training:
                    x = residual + self.dropout(self.self_attn(tgt_q, tgt, tgt, tgt_q_mask)) / (1 - self.death_rate)
                else:
                    x = residual + self.dropout(self.self_attn(tgt_q, tgt, tgt, tgt_q_mask))
        if not self.normalize_before:
            x = self.norm1(x)
        ...
        if self.concat_after:
            x_concat = torch.cat((x, self.src_attn(x, memory, memory, memory_mask)), dim=-1)
            if not coin:
                x = residual + self.concat_linear2(x_concat) * 0.0
            else:
                if self.training:
                    x = residual + self.concat_linear2(x_concat) / (1 - self.death_rate)
                else:
                    x = residual + self.concat_linear2(x_concat)
        else:
            if not coin:
                x = residual + self.dropout(self.src_attn(x, memory, memory, memory_mask)) * 0.0
            else:
                if self.training:
                    x = residual + self.dropout(self.src_attn(x, memory, memory, memory_mask)) / (1 - self.death_rate)
                else:
                    x = residual + self.dropout(self.src_attn(x, memory, memory, memory_mask))
        if not self.normalize_before:
            x = self.norm2(x)
