...
class Encoder(torch.nn.Module):
    ...
    def __init__(self, idim,
        ...
        if input_layer == "linear":
            ...
            self.embed = Conv2dSubsampling_1layer_with_deltas(idim, attention_dim, dropout_rate, delta=True)
        elif input_layer == "conv2d_with_deltas":
            self.embed = Conv2dSubsampling_with_deltas(idim, attention_dim, dropout_rate)
        elif input_layer == "conv2d_yingbo":
            self.embed = Conv2dSubsampling_yingbo(idim, attention_dim, dropout_rate)
        ...
        self.encoders = repeat_i(
            num_blocks,
            lambda i: StochasticEncoderLayer(
                attention_dim,
    ...
    def forward(self, xs, masks):
        ...
        """
        if isinstance(self.embed, Conv2dSubsampling) or isinstance(self.embed, Conv2dSubsampling_with_deltas) \
                or isinstance(self.embed, Conv2dSubsampling_1layer_with_deltas) \
                or isinstance(self.embed, Conv2dSubsampling_yingbo):
